...
class DatabaseManager:
    ...
    def _inicializar_db(self):
        with self._conectar() as conn:
            ...
            self._migrar(cur, "clientes", {
                "direccion_completa": "TEXT", "barrio_sector": "TEXT",
                "estrato": "INTEGER", "geolocalizacion": "TEXT",
            ...
            self._migrar(cur, "proyectos", {
                "nombre_proyecto": "TEXT", "fecha_instalacion": "DATE",
                "kwp_instalados": "REAL", "marca_paneles": "TEXT",
                "seriales_paneles": "TEXT", "marca_inversor": "TEXT",
                "serial_inversor": "TEXT", "capacidad_inversor_kw": "REAL DEFAULT 3.0",
                "estatus_retie": "TEXT DEFAULT 'Pendiente'", "ruta_fotos_entrega": "TEXT",
            })
            cur.execute("""
            ...
            self._migrar(cur, "consumos", {
                ...
                "kwh_generados_gsc":      "REAL DEFAULT 0",
                "cargos_adicionales":     "REAL DEFAULT 0",
                "descuentos_adicionales": "REAL DEFAULT 0",
                "consumo_total_periodo":  "REAL DEFAULT 0",
                "dias_ciclo":             "INTEGER DEFAULT 30",
            })
            cur.execute("""
    ...
    def obtener_clientes(self, busqueda=""):
        with self._conectar() as conn:
            if busqueda:
                like = f"%{busqueda}%"
                rows = conn.execute(
                    "SELECT * FROM clientes WHERE nombre_titular LIKE ? "
                    "OR cedula_nit LIKE ? OR telefono LIKE ? ORDER BY nombre_titular",
                    (like, like, like)).fetchall()
            else:
                rows = conn.execute("SELECT * FROM clientes ORDER BY nombre_titular").fetchall()
            return [dict(r) for r in rows]

    def obtener_cliente_por_id(self, cid):
        with self._conectar() as conn:
    ...
    def crear_cliente(self, d):
        with self._conectar() as conn:
            cur = conn.execute("""
                INSERT INTO clientes (nombre_titular, cedula_nit, direccion_completa,
                    barrio_sector, estrato, geolocalizacion, tarifa_aire_actual,
                    tarifa_gsc, telefono, email)
                VALUES (?,?,?,?,?,?,?,?,?,?)""",
                (d.get("nombre"), d.get("nit"), d.get("dir"), d.get("barrio"),
                 d.get("estrato") or None, d.get("geo"),
                 d.get("tarifa") or 0, d.get("tarifa_gsc") or 0,
                 d.get("tel"), d.get("mail")))
            conn.commit()
            return cur.lastrowid

    def actualizar_cliente(self, cid, d):
        with self._conectar() as conn:
            conn.execute("""
                UPDATE clientes SET nombre_titular=?, cedula_nit=?, direccion_completa=?,
                    barrio_sector=?, estrato=?, geolocalizacion=?, tarifa_aire_actual=?,
                    tarifa_gsc=?, telefono=?, email=? WHERE id=?""",
                (d.get("nombre"), d.get("nit"), d.get("dir"), d.get("barrio"),
                 d.get("estrato") or None, d.get("geo"),
                 d.get("tarifa") or 0, d.get("tarifa_gsc") or 0,
                 d.get("tel"), d.get("mail"), cid))
            conn.commit()
        ...
        with self._conectar() as conn:
            if cliente_id:
                rows = conn.execute("""
                    ...
                    WHERE p.cliente_id=? ORDER BY p.id DESC""", (cliente_id,)).fetchall()
            else:
                rows = conn.execute("""
                    SELECT p.*, c.nombre_titular as cliente_nombre,
                           c.tarifa_aire_actual, c.tarifa_gsc
    ...
    def obtener_proyecto_por_id(self, pid):
        with self._conectar() as conn:
            r = conn.execute("""
                SELECT p.*, c.nombre_titular as cliente_nombre,
                       c.tarifa_aire_actual, c.tarifa_gsc
    ...
    def crear_proyecto(self, d):
        ...
        with self._conectar() as conn:
            cur = conn.execute("""
                INSERT INTO proyectos (cliente_id, nombre_proyecto, fecha_instalacion,
                    kwp_instalados, marca_paneles, seriales_paneles, marca_inversor,
                    serial_inversor, capacidad_inversor_kw, estatus_retie, ruta_fotos_entrega)
                VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
                (d.get("cliente_id"), d.get("nombre"),
                 d.get("fecha_instalacion") or _dt.date.today().isoformat(),
    ...
    def actualizar_proyecto(self, pid, d):
        with self._conectar() as conn:
            conn.execute("""
                UPDATE proyectos SET nombre_proyecto=?, fecha_instalacion=?,
                    kwp_instalados=?, marca_paneles=?, seriales_paneles=?,
                    marca_inversor=?, serial_inversor=?, capacidad_inversor_kw=?,
                    estatus_retie=?, ruta_fotos_entrega=? WHERE id=?""",
        ...
        with self._conectar() as conn:
            if cliente_id:
                rows = conn.execute("""
                    ...
                    FROM consumos co JOIN clientes c ON co.cliente_id=c.id
                    WHERE co.cliente_id=? ORDER BY co.anio DESC, co.mes DESC""",
                    (cliente_id,)).fetchall()
            else:
                rows = conn.execute("""
                    SELECT co.*, c.nombre_titular, c.tarifa_aire_actual, c.tarifa_gsc
                    FROM consumos co JOIN clientes c ON co.cliente_id=c.id
                    ORDER BY co.anio DESC, co.mes DESC""").fetchall()
            return [dict(r) for r in rows]

    def crear_consumo(self, d):
        with self._conectar() as conn:
            cur = conn.execute("""
                INSERT INTO consumos (
                    cliente_id, mes, anio, kwh_consumidos, lectura_medidor_red,
                    subsidio_aire, deducciones_aire, fecha_inicio_ciclo, fecha_fin_ciclo,
                    kwh_generados_gsc, cargos_adicionales, descuentos_adicionales,
                    consumo_total_periodo, dias_ciclo
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (d["cliente_id"], d["mes"], d["anio"],
                 ...
                 d.get("fecha_inicio_ciclo"), d.get("fecha_fin_ciclo"),
                 d.get("kwh_generados_gsc", 0), d.get("cargos_adicionales", 0),
                 d.get("descuentos_adicionales", 0),
                 d.get("consumo_total_periodo", 0), d.get("dias_ciclo", 30)))
            conn.commit()
            return cur.lastrowid

    # ── MANTENIMIENTOS ────────────────────────────────────────────────────────

    def obtener_mantenimientos(self, proyecto_id=None):
        with self._conectar() as conn:
            if proyecto_id:
                rows = conn.execute("""
                    ...
                    WHERE m.proyecto_id=? ORDER BY m.fecha_visita DESC""",
                ...
                rows = conn.execute("""
                    ...
                    JOIN proyectos p ON m.proyecto_id=p.id
                    JOIN clientes c ON p.cliente_id=c.id
                    ORDER BY m.fecha_visita DESC""").fetchall()
            return [dict(r) for r in rows]

    def obtener_mantenimiento_por_id(self, mid):
        with self._conectar() as conn:
            r = conn.execute("""
                SELECT m.*, c.nombre_titular
                FROM mantenimientos m
    ...
    def crear_mantenimiento(self, d):
        with self._conectar() as conn:
            cur = conn.execute("""
                INSERT INTO mantenimientos (
                    proyecto_id, fecha_visita, tecnico_encargado,
                    estado_estructura, estado_cableado, limpieza_paneles,
                    continuidad_tierra_ohm, ruta_fotos, ruta_acta_pdf, observaciones_criticas
                ) VALUES (?,?,?,?,?,?,?,?,?,?)""",
    ...
    def obtener_kpis(self):
        with self._conectar() as conn:
            kwp = conn.execute(
                "SELECT COALESCE(SUM(kwp_instalados),0) FROM proyectos").fetchone()[0]
            ...
            retie_pend = conn.execute(
                "SELECT COUNT(*) FROM proyectos WHERE estatus_retie='Pendiente'").fetchone()[0]
            alertas = conn.execute("""
                ...
                GROUP BY p.id
                HAVING ultimo_mant IS NULL OR date(ultimo_mant) <= date('now','-6 months')
                ORDER BY ultimo_mant ASC LIMIT 8""").fetchall()
            return {
                "kwp": round(kwp, 2), "clientes": clientes,
                "proyectos": proyectos, "retie_pend": retie_pend,
    ...
    def obtener_datos_grafico_cliente(self, cliente_id):
        with self._conectar() as conn:
            rows = conn.execute("""
                SELECT mes, anio, kwh_consumidos, kwh_generados_gsc,
                       consumo_total_periodo, dias_ciclo
                FROM consumos WHERE cliente_id=? ORDER BY anio, mes LIMIT 12""",
                (cliente_id,)).fetchall()
            return [dict(r) for r in rows]
    ...
    def obtener_historial_cliente(self, cliente_id):
        ...
        for r in rows:
            ...
            r["lectura_inversor_gsc"] = r.get("kwh_generados_gsc", 0)
